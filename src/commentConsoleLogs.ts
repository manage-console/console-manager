...
export function registerCommentConsoleLogsCommand(
  context: vscode.ExtensionContext,
) {
  let disposable = vscode.commands.registerCommand(
    "extension.commentConsoleLogs",
    async () => {
      ...
      if (editor) {
        ...
        if (text !== commentedText) {
          ...
          vscode.window.showInformationMessage(
            "Console.log statements have been commented out✨✅",
          );
        } else {
          vscode.window.showInformationMessage(
            "No console.log statements found to comment ❗",
          );
        }
      }
    },
